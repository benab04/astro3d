...
function createMoon({
    ...
    initialRotation = -1 * Math.PI / 2, // Initial rotation of the sphere
} = {}) {
    // Create geometry
    ...
    const moonMaterial = new THREE.MeshPhysicalMaterial({
        map: colorTexture,                   // Base texture of the Moon
        normalMap: normalTexture,           // Normal map for surface details
        normalScale: new THREE.Vector2(normalScale, normalScale), // Adjusted for subtle surface roughness
        roughness: roughness,                     // High roughness to mimic lunar regolith
        metalness: metalness,                     // No metallic reflections for the Moon
        reflectivity: reflectivity,                 // Minimal reflectivity to reduce gloss
        clearcoat: clearcoat,                     // No clearcoat for a natural appearance
        color: color,   // Grayish tone for the Moon's surface

        aoMapIntensity: aoMapIntensity,                // Slightly higher ambient occlusion for depth
        lightMapIntensity: lightMapIntensity,             // Balance light interactions
        envMapIntensity: envMapIntensity,              // Low environment map intensity to reduce shine

        side: side,              // Render only the front faces
        transparent: transparent,                 // Opaque material
        flatShading: flatShading,                 // Smooth shading for realism

        bumpMap: normalTexture,             // Use normal map as bump map for extra detail
        bumpScale: bumpScale,                   // Subtle bump effect
    });
